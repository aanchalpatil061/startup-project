...
export default function AdminSettingsScreen() {
  ...
  const changePwMutation = useMutation({
    ...
    onSuccess: () => {
      Alert.alert('Password Changed', 'Admin password updated successfully.');
  ...
  const handleChangePassword = () => {
    if (!currentPassword || newPassword.length < 8 || newPassword !== confirmNew) {
      Alert.alert('Validation Error', 'Check all fields. New password must be 8+ characters and match.');
      return;
    }
    ...
    <View style={[styles.flex, { paddingTop: insets.top }]}>
      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 24 }]} showsVerticalScrollIndicator={false}>
        ...
        <Card padding={0}>
          ...
          {showChangePass && (
            <View style={styles.changePassForm}>
              ...
              <Input label="Confirm New Password" value={confirmNew} onChangeText={setConfirmNew} password leftIcon="lock-open-outline" />
        ...
        <Card padding={0}>
          ...
          <InfoRow label="Platform" value="Askd Admin Console" />
          <View style={styles.divider} />
          <InfoRow label="Support" value="support@askd.in" />
          <View style={styles.divider} />
          <InfoRow label="API Endpoint" value={process.env.EXPO_PUBLIC_API_URL ?? 'api.askd.in'} />
        </Card>
      </ScrollView>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  infoValue: { fontSize: FontSize.sm, fontFamily: FontFamily.regular, color: Colors.textSecondary, maxWidth: '50%' },
});
