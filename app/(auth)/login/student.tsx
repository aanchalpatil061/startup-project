...
import { isValidEmail, isValidRollNumber } from '../../../src/utils/validation';

type IdentifierType = 'email' | 'rollNumber';
...
export default function StudentLoginScreen() {
  ...
  const loginMutation = useMutation({
    ...
    onError: (err: any) => {
      Alert.alert('Sign In Failed', err?.response?.data?.message ?? 'Invalid credentials. Please try again.');
    },
  });
  ...
  const validate = () => {
    ...
    if (!identifier.trim()) {
      errs.identifier = identifierType === 'email' ? 'Email is required' : 'Roll number is required';
    } else if (identifierType === 'email' && !isValidEmail(identifier)) {
      errs.identifier = 'Enter a valid email address';
    } else if (identifierType === 'rollNumber' && !isValidRollNumber(identifier)) {
      errs.identifier = 'Enter a valid roll number';
    }
    ...
    >
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>Sign in to view your evaluated scripts</Text>
        </View>

        <View style={styles.tabs}>
          {(['email', 'rollNumber'] as IdentifierType[]).map((t) => (
            <TouchableOpacity
              key={t}
            ...
            >
              <Text style={[styles.tabText, identifierType === t && styles.activeTabText]}>
                {t === 'email' ? 'College Email' : 'Roll Number'}
              </Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <View style={styles.form}>
          <Input
            label={identifierType === 'email' ? 'College Email' : 'Roll Number'}
            placeholder={identifierType === 'email' ? 'you@college.ac.in' : 'e.g. 21CS001'}
            value={identifier}
            onChangeText={setIdentifier}
            keyboardType={identifierType === 'email' ? 'email-address' : 'default'}
            autoCapitalize="none"
            leftIcon={identifierType === 'email' ? 'mail-outline' : 'id-card-outline'}
        ...
        <View style={styles.signupRow}>
          ...
          <TouchableOpacity onPress={() => router.push('/(auth)/signup/student')}>
