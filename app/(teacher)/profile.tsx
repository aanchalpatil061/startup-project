  ...
  return (
    ...
    >
      ...
      <View style={styles.profileCard}>
        <Avatar name={teacher.name} size={72} uri={teacher.avatar} />
        <View style={styles.profileInfo}>
          <Text style={styles.name}>{teacher.name}</Text>
          <Text style={styles.email}>{teacher.email}</Text>
          <Text style={styles.role}>Faculty · {teacher.department}</Text>
        </View>
      </View>

      <Card padding={0}>
        <InfoRow label="Employee ID" value={teacher.employeeId} icon="briefcase-outline" />
        <Divider />
        <InfoRow label="Department" value={teacher.department} icon="school-outline" />
        <Divider />
        <InfoRow label="Email" value={teacher.email} icon="mail-outline" />
      </Card>

      <Card padding={0}>
        <MenuItem icon="settings-outline" label="Settings" onPress={() => router.push('/(teacher)/settings')} />
        <Divider />
        <MenuItem icon="notifications-outline" label="Notifications" onPress={() => router.push('/(teacher)/notifications')} />
        <Divider />
